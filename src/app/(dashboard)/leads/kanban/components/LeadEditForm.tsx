...
import { CalendarIcon, RefreshCcw, UserX } from "lucide-react"
import {
  SheetContent,
  SheetHeader,
  SheetTitle,
...
export function LeadSheet({ lead }:Props) {
  ...
  const getStatusColor = (status:LeadStatus) => {
    const statusColors = {
      [LeadStatus.Contacto]: "bg-gray-200",
    ...
    <SheetContent className="sm:max-w-md overflow-y-auto">
      <div className="flex flex-col h-full">
        ...
        <div className="flex w-[90%] justify-between">
          <SheetTitle className="text-xl font-bold">{lead?.empresa || "Información del Lead"}</SheetTitle>
          <RefreshCcw size={20} onClick={() => router.refresh()} className="cursor-pointer"/>
        </div>
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="text-sm text-muted-foreground">
              Creado: {lead?.createdAt? format(new Date(lead.createdAt), "dd MMM yyyy", { locale: es }) : "N/A"}
            </span>
          </div>
        </SheetHeader>
        ...
        <div className="py-4 border-b">
          <div className="grid grid-cols-1 gap-3 w-full">
            <div className="flex justify-between">
              ...
              <Badge variant="outline">
                <p className="text-muted-foreground">{lead?.sector || "No especificado"}</p>
              </Badge>
            </div>
            <div className="flex justify-between">
              <h3 className="text-sm font-medium">Origen</h3>
              <Badge variant="outline">
              <p className="text-muted-foreground">{lead?.origen || "No especificado"}</p>
              </Badge>
            </div>
            <div className="flex justify-between">
              ...
              <Badge variant="outline">
                ...
                <span className="text-muted-foreground">{fechaConectar ? format(fechaConectar, "dd MMM yyyy", { locale: es }) : "No establecida"}</span>
              </Badge>
              </div>
            </div>
            <div className="flex justify-between">
              ...
              <Badge variant="outline">
                <CalendarIcon className="h-3 w-3 mr-2" />
                <span className="text-muted-foreground">{fechaProspectar ? format(fechaProspectar, "dd MMM yyyy", { locale: es }) : "No establecida"}</span>
              </Badge>
              </div>
            </div>
            <div className="flex justify-between">
              ...
              <Badge variant="outline">
              <p className="text-muted-foreground">{diffInDays} dias</p>
              </Badge>
            </div>
            <div className="flex justify-between">
              <h3 className="text-sm font-medium">Generador</h3>
              <Badge variant="outline">
              <p className="text-muted-foreground">{lead.generadorLeads.name}</p>
              </Badge>
            </div>
          </div>
          ...
          {lead?.link && (
            <div className="mt-3">
              ...
              <a href={lead.link} className="text-blue-600 hover:underline truncate block" target="_blank" rel="noopener noreferrer">
                {lead.link}
              </a>
            </div>
          )}
        ...
        <div className="flex-grow overflow-y-auto">
          <Tabs defaultValue="contacts" className="mt-4">
              ...
              {lead?.contactos?.length > 0 ? (
                <div className="space-y-4">
                  {lead.contactos.map((contacto, index) => (
                    <ContactoCard
                    contacto={contacto}key={contacto.id}
                    />
                  ))}
                </div>
              ) : (
            ...
            <ScrollArea className="h-80 rounded-md border p-3 z-50">
              {lead?.statusHistory?.length > 0 ? (
                <div className="space-y-3">
                  {lead.statusHistory.map((item, index) => (
                    <div key={index} className="flex justify-between items-center text-sm border-b pb-3">
                      <div>
                        <span className="font-medium">{leadStatusMap[item.status]}</span>
                        <p className="text-muted-foreground text-xs">
                          Por: {item.changedBy.name}
                        </p>
                      </div>
                      <span className="text-xs text-muted-foreground">
                        {format(new Date(item.changedAt), "dd/MM/yy HH:mm", { locale: es })}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <h3 className="font-medium mb-1">Sin historial de cambios</h3>
